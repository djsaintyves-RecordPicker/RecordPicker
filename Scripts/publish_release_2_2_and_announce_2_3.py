...
def promote_gallery(text: str) -> str:
    gallery = re.search(
        r'<section\b[^>]*data-release-gallery="(?:2\.1\.1|2\.0)"[^>]*>.*?</section>',
        text,
        flags=re.DOTALL,
    )
    if not gallery:
        return text
    promoted = re.sub(
        r'data-release-gallery="(?:2\.1\.1|2\.0)"',
        'data-release-gallery="2.2"', gallery.group(0), count=1,
    )
    promoted = re.sub(
        r'<h2>Record Picker (?:2\.1\.1|2\.0)</h2>',
        '<h2>Record Picker 2.2</h2>', promoted, count=1,
    )
...
def update_release_pages(root: Path, note_locale: str) -> int:
    ...
    if not release_block(text, "2.3", "section"):
        previous_next = release_block(text, "2.2", "section") or release_block(text, "2.1", "section")
        previous_current = release_block(text, "2.1.1", "section") or release_block(text, "2.0", "section")
        if not previous_next or not previous_current:
            raise RuntimeError(f"Expected current and upcoming home blocks in {home_path}")
    ...
    original = text
    if not release_block(text, "2.3", "article"):
        old_21 = release_block(text, "2.2", "article")
        current_20 = release_block(text, "2.1.1", "article") or release_block(text, "2.0", "article")
        if not old_21 or not current_20:
            raise RuntimeError(f"Expected current and upcoming history cards in {history_path}")
        ...
        historical_21 = re.sub(
            r'\s*<p class="release-platform-summary">.*?</p>', "", current_20.group(0),
        ...
        text = text[:old_21.start()] + replacement + text[current_20.end():]
    duplicate_22 = list(re.finditer(
        r'<article\b[^>]*data-release-version="2\.2"[^>]*>.*?</article>',
        text, flags=re.DOTALL,
    ))
    for duplicate in reversed(duplicate_22[1:]):
        text = text[:duplicate.start()] + text[duplicate.end():]
    for historical_version in ("2.1.1", "2.1", "2.0"):
    ...
    original = text
    if not release_block(text, "2.3", "section"):
        old_marker = release_block(text, "2.2", "section") or release_block(text, "2.1", "section")
        if not old_marker:
...
def update_shared_metadata(path: Path) -> bool:
    ...
    original = text
    text = re.sub(r'("softwareVersion":")2\.(?:0|1(?:\.1)?|2)(")', r'\g<1>2.2\2', text)
    text = re.sub(r'("dateModified":")[^"]+(")', rf'\g<1>{PUBLICATION_DATE}\2', text)
    text = re.sub(r'Record Picker v2\.(?:0|1(?:\.1)?)</span>', 'Record Picker v2.2</span>', text)
    text = re.sub(
        r'(<p class="doc-meta">.*?)Record Picker v?2\.(?:0|1(?:\.1)?)(.*?</p>)',
        r'\1Record Picker v2.2\2', text, flags=re.DOTALL,
    )
    text = re.sub(
        r'(<p class="glass-pill eyebrow">.*?)macOS 2\.(?:0|1(?:\.1)?)(.*?</p>)',
        r'\1macOS 2.2\2', text, flags=re.DOTALL,
    )
    text = re.sub(
        r'(<p class="eyebrow">.*?)Record Picker 2\.(?:0|1(?:\.1)?)(.*?</p>)',
        r'\1Record Picker 2.2\2', text, flags=re.DOTALL,
    )
    text = text.replace("<h2>iOS 2.1.1 · macOS 2.2</h2>", "<h2>Record Picker 2.2</h2>")
    if text == original:
        return False
...
def update_state() -> None:
    ...
    state["current_release"] = {
        "version": "2.2",
        "platform_versions": {
            "iphone": "2.2", "ipad": "2.2", "mac": "2.2", "watch": "2.2",
        },
        "platforms": {
    ...
    state["historical_releases"] = [
        "2.1.1", "1.9", "1.8", "1.6", "1.5", "1.4", "1.3", "1.2", "1.1"
    ]
